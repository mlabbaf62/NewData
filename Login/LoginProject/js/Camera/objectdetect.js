...
var objectdetect = (function () {
    ...
    var /**
		...
		groupRectangles = function (rects, minNeighbors, confluence) {
		    ...
		    for (var i = numClasses - 1; i >= 0; --i) {
		        var group = groups[i];
		        if (group[4] >= minNeighbors) {
		            var numNeighbors = 1 / group[4];
    ...
    var detector = (function () {
        ...
        detector.prototype.detect = function (imageData, group, stepSize, roi, canny) {
            if (stepSize === undefined) stepSize = 1;
            if (group === undefined) group = 1;

            //var width = this.canvas.width;
            //var height = this.canvas.height;
            var width = this.imageDataWidth;
            var height = this.imageDataHeight;
