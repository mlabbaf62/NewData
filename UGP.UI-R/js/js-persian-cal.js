...
AMIB.persianCalendar = function (b, a, pCalType) {
    //add by sh
    this.CalType = pCalType;
    $("#" + b).attr("placeholder", "تاریخ را وارد کنید");
    if (pCalType == 'p') {
        var c = {
            inputID: "",
            extraInputID: "",
    ...
    //////////////////
    //add by sheikhi
    this.GenrateCal = function (f, g, A, B, dday) {

        var currentDate = B.toString() + "/" + padLeft(A, 2) + "/" + padLeft(dday, 2);

        var tmpVacationDate = Enumerable.From(this.Holiday)
            .Where(function (x) {
                return (x == currentDate)
            })
            .FirstOrDefault()
        if (tmpVacationDate != null) {
            f.className = ' weekday DeActiveDay Holiday';
            f.title = "تعطیل رسمی";
        }
        ...
        if (this.CalType == 'p') {
            //f.className += ' ActiveDay'; //f.className += " ActiveDay"; //f.classList.add("ActiveDay");
            if (f.className.indexOf('friday') == -1)
                f.className += ' ActiveDay'; //f.className += " ActiveDay"; //f.classList.add("ActiveDay");
            if (this.RevisitDate != undefined) {
                var tmpRDate = Enumerable.From(this.RevisitDate)
                    .Where(function (x) { return (x.RevisitDate == currentDate) })
                    .FirstOrDefault()
            }
        }
        else {
            if (this.EnableDays != undefined && this.EnableDays.indexOf(currentDate) == -1) {//this.CalType == 'p'
                f.className += ' DeActiveDay'; //f.className += " DeActiveDay";//f.classList.add();
            } else {
                if (f.className.indexOf('friday') == -1)
                    f.className += ' ActiveDay'; //f.className += " ActiveDay"; //f.classList.add("ActiveDay");
            }
            ...
            if (tmpRDate != null && f.className.indexOf('DeActiveDay') == -1) {
                f.className += ' MiniActive ';
                f.title = "تعداد بازدید قابل انجام : " + tmpRDate.MaxVisit + "\r\n" + "تعداد بازدید اعلام شده در این روز : " + tmpRDate.CountInDay;
            }

            var tmpVacationDate = Enumerable.From(this.VacationDate)
                .Where(function (x) {
                    return (x.VacationDate == currentDate)
                })
                .FirstOrDefault()
            if (tmpVacationDate != null) {
                f.className = ' weekday DeActiveDay VacationDate';
                f.title = "مرخصی";
            }

            if ((this.DisableDays != undefined && this.DisableDays.indexOf(currentDate) > -1) || f.className.indexOf('friday') > -1) {
                f.className += ' DeActiveDay ';
            }
            //else {

            //    f.className += ' DeActiveDay '; //f.className += " ActiveDay"; //f.classList.add("ActiveDay");
            //}
        }
        f.appendChild(cTN(enDigitsToFa(dday))), g.appendChild(f);
        $(f).on("click", function (pitem) {
            $('.weekday').removeClass('selected');
            $(pitem.currentTarget).addClass('selected');
        });
    }
    ///////////////////////
    this.input = A$(b);
    ...
    d.onclick = function (a) {
        f.showHidePicker(a);
        //GetMonth(); CheckDate(); console.log("_pDatePicker clicked !");//added
        return !1
    };
    var h = this;
    d.onblur = function () {
...
AMIB.persianCalendar.prototype.showHidePicker = function (b) {
    ...
    if (c = A$(a)) {
        ...
        //this.inputID+"_pDatePicker"
        //document.getElementById(this.inputID + "_pDatePicker").appendChild(c);
        $("#" + this.inputID).after(c);
...
AMIB.persianCalendar.prototype.createDateTable = function (b, a, c) {
    ...
    f.onclick = function (a) {
        ...
        //GenrateCal(f, g, a[1], a[0], a[2])
        //CheckDate(); console.log("_pDatePicker clicked !");//?
    ...
    for (k = persianMonthDays(a[0], a[1]) + l; h < k; ++h) 0 == h % 7 && (e = d.insertRow(-1)),
        g = e.insertCell(-1), f = cE("a"),
        f.href = "#",
        a[2] = h - l + 1,
        f.className = "weekday",
        ...
        f.onclick = function (a, b) {
            //MainDate = null;
            return function () {
                a.fillDateField(b); //MainDate = b;
                return !1
            }

        }(this, a.join("/")),
        this.GenrateCal(f, g, a[1], a[0], a[2]);
    //GenrateCal(f, g, A, B, a[2]);
...
AMIB.persianCalendar.prototype.SetEnableDate = function (pdates) {
    this.EnableDays = pdates;
}
AMIB.persianCalendar.prototype.SetDisableDate = function (pdates, pRevisitDate, pVacationDate) {
    this.DisableDays = pdates;
    this.RevisitDate = pRevisitDate;
    this.VacationDate = pVacationDate;
}
AMIB.persianCalendar.prototype.SetHoliday = function () {
    //تعطیلات تقویم
    ///////////////////////////////////////////
    ...
    this.Holiday.push(pYear + "/01/01");
    this.Holiday.push(pYear + "/01/02");
    this.Holiday.push(pYear + "/01/03");
    this.Holiday.push(pYear + "/01/04");
    this.Holiday.push(pYear + "/01/12");
    this.Holiday.push(pYear + "/01/13");
    this.Holiday.push(pYear + "/03/14");//رحلت امام
    this.Holiday.push(pYear + "/03/15");
    this.Holiday.push(pYear + "/11/22");
    this.Holiday.push(pYear + "/12/29");

    if (DateHoliday != 'undefined')
        this.Holiday = this.Holiday.concat(DateHoliday);
    ...
    if (A$(this.inputID + "_pDatePicker")) {
        ...
        for (d = 0; 12 > d; ++d) g = cE("a"), g.href = "#", g.appendChild(cTN(PERSIAN_MONTH_NAMES[d])), d == a - 1 && (g.className = "selected"), g.onclick = function (a, b, c) {
            return function (d) {
                ...
                //CheckDate(); console.log("showMonthPicker a clicked !");//added
                return !1
            }
        }(this,
...
AMIB.persianCalendar.prototype.showYearPicker = function (b, a, c) {
    ...
    if (A$(this.inputID + "_pDatePicker")) {
        ...
        for (d = c - 5; d <= c + 5; ++d) g = cE("a"), g.href = "#", g.appendChild(cTN(enDigitsToFa(d))), d == c && (g.className = "selected"), g.onclick = function (a, b, c) {
            return function (d) {
                a.monthYearPickerClick(d, b, c);
                //CheckDate(c); console.log("showYearPicker a clicked !");//added
...
AMIB.persianCalendar.prototype.fillDateField = function (b) {
    ...
    b[1] = padLeft(b[1], 2);//add by sheikhi
    b[2] = padLeft(b[2], 2);//add by sheikhi
    a.value = b.join("/");
    ...
    try {
        ...
        DateBazdidPlugin.DateClickEvent(a.value, this.inputID);//tDate
    }
    catch (ex) { }
};
